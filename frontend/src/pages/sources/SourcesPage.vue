...
const isTdx = computed(() => form.value.provider === "tdx");
...
const canSave = computed(() => {
  ...
  if (isTdx.value && !form.value.base_url.trim()) {
    return false;
  }
...
function buildConfig() {
  ...
  if (isTushare.value && form.value.token.trim()) {
    config.token = form.value.token.trim();
  }
  if (isTushare.value) {
    const pts = Number(form.value.account_points);
  ...
  if (isTdx.value) {
    config.base_url = form.value.base_url.trim();
    if (form.value.api_token.trim()) config.api_token = form.value.api_token.trim();
    if (form.value.install_root.trim()) config.install_root = form.value.install_root.trim();
    config.import_mode = form.value.import_mode;
    const paths: Record<string, string> = {};
    if (form.value.vipdoc_root.trim()) paths.vipdoc_root = form.value.vipdoc_root.trim();
    if (form.value.hq_cache_root.trim()) paths.hq_cache_root = form.value.hq_cache_root.trim();
...
function startEdit(source: DataSource) {
  ...
  const paths = (source.config.paths as Record<string, string> | undefined) || {};
  form.value = {
    name: source.name,
    ...
    base_url: String(source.config.base_url || "http://127.0.0.1:8090"),
    api_token: "",
    install_root: String(source.config.install_root || ""),
    import_mode: String(source.config.import_mode || "file_first"),
    vipdoc_root: String(paths.vipdoc_root || ""),
    hq_cache_root: String(paths.hq_cache_root || ""),
  };
}
...
async function verifySource(sourceId: number, provider: string) {
  ...
  try {
    ...
    const quotaMsg = res.quota
      ...
      : "";
    const probeMsg =
      provider === "tdx" && res.probe
        ? ` · .day 文件 ${res.probe.lday_file_count ?? "—"}`
        : "";
    ui.showMessage(res.message + quotaMsg + probeMsg, res.ok ? "success" : "error");
  } catch (e) {
    ui.showMessage(e instanceof Error ? e.message : String(e), "error");
  } finally {
...
async function probeTdx(sourceId?: number) {
  if (!auth.isAdmin || probing.value) return;
  probing.value = true;
  try {
    const body: Record<string, unknown> = {};
    if (sourceId) body.source_id = sourceId;
    else {
      body.base_url = form.value.base_url.trim();
      if (form.value.api_token.trim()) body.api_token = form.value.api_token.trim();
      if (form.value.install_root.trim()) body.install_root = form.value.install_root.trim();
      const paths: Record<string, string> = {};
      if (form.value.vipdoc_root.trim()) paths.vipdoc_root = form.value.vipdoc_root.trim();
      if (form.value.hq_cache_root.trim()) paths.hq_cache_root = form.value.hq_cache_root.trim();
      if (Object.keys(paths).length) body.paths = paths;
    }
    const res = await apiRequest<{ ok: boolean; message: string; status: Record<string, unknown> }>(
      "/api/v1/sources/tdx-probe",
      { method: "POST", body: JSON.stringify(body) },
    );
    const count = res.status?.lday_file_count ?? "—";
    ui.showMessage(`${res.message}（.day 文件 ${count}）`, res.ok ? "success" : "error");
  } catch (e) {
    ui.showMessage(e instanceof Error ? e.message : String(e), "error");
  } finally {
    probing.value = false;
...
<template>
  <PageHeader
    title="数据源"
    description="Tushare 须配置账户积分；TDX 须配置 Sidecar 与通达信安装路径"
  />
  ...
  <div v-if="auth.isAdmin" class="panel">
    ...
    <div class="panel__body page-toolbar">
      ...
      <template v-if="isTushare">
        ...
        <div class="btn-group">
          <button type="button" class="btn btn--ghost btn--sm" @click="setPointsPreset(120)">120</button>
          <button type="button" class="btn btn--ghost btn--sm" @click="setPointsPreset(2000)">2000</button>
          <button type="button" class="btn btn--ghost btn--sm" @click="setPointsPreset(5000)">5000</button>
        </div>
      </template>
      ...
      <template v-if="isTdx">
        <label class="form-field">
          ...
          <input v-model="form.base_url" class="input-w-lg" placeholder="http://127.0.0.1:8090" />
        </label>
        <label class="form-field">
          <span>Sidecar Token</span>
          <input
        ...
        <label class="form-field">
          <span>安装路径</span>
          <input v-model="form.install_root" class="input-w-lg" placeholder="C:\new_tdx" />
        </label>
        <label class="form-field">
          <span>导入模式</span>
          <select v-model="form.import_mode" class="input-w-sm">
            <option value="file_first">文件优先</option>
            <option value="network">仅网络</option>
          </select>
        </label>
        <label class="form-field">
          <span>vipdoc 路径</span>
          <input v-model="form.vipdoc_root" class="input-w-lg" placeholder="可选，覆盖默认" />
        </label>
        <label class="form-field">
          <span>hq_cache 路径</span>
          <input v-model="form.hq_cache_root" class="input-w-lg" placeholder="可选，概念板块" />
        </label>
        <button
          type="button"
          class="btn btn--ghost"
    ...
    <p v-if="isTdx" class="panel__hint">
      T+1 行情经 Sidecar 读取 vipdoc；实时行情仍由 xtqmt Sidecar 负责。
    </p>
  </div>
      ...
      <table v-if="sources.length" class="data-table">
        ...
        <tbody>
          <tr v-for="s in sources" :key="s.id">
            ...
            <td>
              <code v-if="s.provider === 'tdx'">{{ s.config.base_url || "—" }}</code>
              <code v-else>{{ s.config.token || "—" }}</code>
            </td>
            <td class="numeric">
              <template v-if="s.provider === 'tushare' && s.quota">
                {{ s.quota.account_points }}
                <span v-if="!s.quota.account_points_from_source" class="badge badge--muted">环境变量</span>
              </template>
              <span v-else>—</span>
            </td>
            <td class="numeric">
            ...
            <td v-if="auth.isAdmin">
              <div class="btn-group">
                <button type="button" class="btn btn--ghost btn--sm" @click="startEdit(s)">编辑</button>
                <button
                  type="button"
                ...
                <button type="button" class="btn btn--ghost btn--sm" @click="deleteSource(s.id)">删除</button>
              </div>
            </td>
          </tr>
        </tbody>
